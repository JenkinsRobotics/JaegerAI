...
class AgentCallbacks:
    ...
    # Tool dispatch — phase ∈ {"start", "done"}. ``data`` is the args on
    # "start" and ``{"elapsed_s": float}`` on "done".
    #
    # There is deliberately no "error" phase: a tool that raises is
    # caught and turned into an ``{"ok": False, "error": ...}`` result
    # fed back to the model, so from the loop's side a failed call still
    # completes. Failure lives in the result, not in the phase.
