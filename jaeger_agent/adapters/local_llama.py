...
class _LlamaChatFacade:
    ...
    def create(self, **kwargs: Any) -> Any:
        ...
        if isinstance(tools, list):
            from jaeger_agent.parsing import schema_sanitizer
        ...
        if self._abort_flag is not None and "logits_processor" not in kwargs:
            ...
            try:
                import llama_cpp
                processor_list = llama_cpp.LogitsProcessorList
            except ModuleNotFoundError:
                # An injected llama-compatible test/dummy does not require
                # the optional llama-cpp package. Real local inference does.
                processor_list = list
            kwargs["logits_processor"] = processor_list([_abort_proc])
...
class LocalLlamaAdapter(OpenAIAdapter):
    ...
    def _strip_tool_call_blocks(text: str) -> str:
        ...
        import re
        from jaeger_agent.dialects import gemma
        out = text
