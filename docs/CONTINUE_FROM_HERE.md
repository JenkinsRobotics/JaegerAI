...
| IDE chat search filters the Gateway-owned session list by title, workspace, or session ID without creating a second search service | `interfaces/ide/media/presentation.js::filterChats`; `media/view.js::renderChats`; `interfaces/ide/tests/chat-search.test.js`; `interfaces/ide/tests/presentation.test.js` | Source and unit tests; installed-host acceptance remains open |
| IDE `/workers` reports the Gateway orchestration worker catalog with availability, capabilities, and probe failures; it does not claim live IDE-panel task execution | `interfaces/ide/media/slash-commands.js`; `interfaces/ide/commands.js::workerLines`; `interfaces/ide/extension.js::info/workers`; `interfaces/ide/media/view.js::runSlash`; `interfaces/ide/tests/slash.test.js` | Source and unit tests; installed-host and live-worker acceptance remains open |
| Gateway-backed IDE orchestration has a truthful read-only Codex CLI transport: the default Codex worker advertises `read_only_enforced` and launches `codex exec --sandbox read-only`; writable public delegation remains rejected | `packages/jaeger-agent/jaeger_agent/delegates/codex/runtime.py::create_read_only_runtime`; `features/ide_orchestration/adapters.py::DelegateRuntimeAdapter`; `features/ide_orchestration/service.py::create_default_orchestration_service`; `packages/jaeger-agent/tests/delegates/test_codex_runtime.py`; `test_ide_orchestration.py::test_cli_adapter_allows_read_only_when_runtime_enforces_it`; `test_ide_orchestration.py::test_default_service_registers_codex_read_only_worker` | Source and unit/integration tests; live-provider, independent effect verification, and installed-host acceptance remain open |
...
- **IDE chat search:** 3 focused Node tests passed, 0 failed; covers case-insensitive title/workspace/session-ID filtering, invalid-input handling, the real webview search input, and the no-match state.
- **IDE worker catalog:** 1 Node test passed, 0 failed; covers `/workers` registration, command parsing, the truthful worker-count summary, capability projection, unavailable-worker errors, and the empty-catalog state.
- **Codex read-only orchestration:** 43 focused tests passed, 0 failed (39 unit plus 4 integration); covers the read-only Codex CLI argument contract, capability-specific admission, unsupported-runtime rejection, existing-conversation capability rejection, and the existing Gateway orchestration admission/cancel/replay contracts.
- **IDE Node:** 145 passed, 0 failed, 1 intentional isolated-Gateway fixture skip, exit 0.
