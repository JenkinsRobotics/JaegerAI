...
| **Production-Path** | `--production-path` | Lifecycle, control-plane, runtime truth, effects, PolicyKernel, state ownership, UPAA production runtime | Canonical WorkState, Gateway/runtime separation, product-default runtime, authority/effect contracts, and isolated production-path UPAA tests. Does **not** prove live WebUI or a live LLM turn. |
