...
| **9. Cognition Modes** | Selectable cognitive modes (ReAct, Deliberate Planning, Self-Refinement, Reflection, Delegation). | **VERIFIED** | `CognitionRouter`, `DeliberatePlanner` (LATS $\ge 3$ candidate plans), `SelfRefineEngine` (generate-critique-revise-validate), `ReflexionStore` | Verified in `dev/tests/test_upaa_production_runtime.py::test_8_deliberate_planning_mode` & `test_1_real_executable_trace`. |
