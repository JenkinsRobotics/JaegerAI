...
def test_session_delete_foreign_profile_blocked_before_mutation(monkeypatch):
    handler = _FakeHandler()
    foreign = _SimpleSession("foreign_delete", profile="other")

    monkeypatch.setattr(routes, "_check_csrf", lambda _handler: True)
    monkeypatch.setattr(
        routes,
        "read_body",
        lambda _handler: {"session_id": "foreign_delete"},
    )
    monkeypatch.setattr(
        routes,
        "get_session",
    ...
    monkeypatch.setattr(
        ...
        lambda _sid: (_ for _ in ()).throw(
            AssertionError("delete mutation path should not run")
        ),
    )
...
def test_session_delete_foreign_cli_only_session_blocked_before_mutation(
    monkeypatch,
):
    handler = _FakeHandler()
    monkeypatch.setattr(routes, "_check_csrf", lambda _handler: True)
    monkeypatch.setattr(
        routes,
        "read_body",
        lambda _handler: {"session_id": "foreign_cli_delete"},
    )
    monkeypatch.setattr(
        routes,
        "get_session",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(KeyError("missing sidecar")),
    )
    monkeypatch.setattr(
        routes,
        "_lookup_cli_session_metadata",
        lambda _sid: {"profile": "other", "source_tag": "cli"},
    )
    monkeypatch.setattr(routes, "_get_active_profile_name", lambda: "default")
    monkeypatch.setattr(
        routes,
...
def test_cli_metadata_lookup_finds_older_session_outside_visible_cap(monkeypatch):
    ...
    monkeypatch.setattr(
        profiles,
        "list_profiles_api",
        lambda: [{"name": "default"}, {"name": "jaeger"}],
    )
    monkeypatch.setattr(
        routes,
        "agent_session_rows_existing",
        lambda ids, profile=None: frozenset(ids) if profile == "jaeger" else frozenset(),
    )

    metadata = routes._lookup_cli_session_metadata("older_cli_session")

    assert metadata["session_id"] == "older_cli_session"
    assert metadata["profile"] == "jaeger"
...
def test_session_delete_jaeger_fails_closed_when_authoritative_cleanup_fails(
    ...
    monkeypatch.setattr(
        routes,
        "read_body",
        lambda _handler: {"session_id": "jaeger_delete"},
    )
    monkeypatch.setattr(
        routes,
        "get_session",
        lambda *_args, **_kwargs: cli_stub,
    )
    monkeypatch.setattr(
        routes,
        "_lookup_cli_session_metadata",
        lambda _sid: {"profile": "jaeger", "source_tag": "cli"},
    )
    ...
    monkeypatch.setattr(
        ...
        "_get_session_agent_lock",
        lambda _sid: (_ for _ in ()).throw(
            AssertionError("mirror deletion should not start")
        ),
    )
...
def test_plain_session_new_invalidates_sidebar_cache(monkeypatch):
    """Ordinary chats must appear immediately without a browser refresh."""
    handler = _FakeHandler()
    events = []
    cleared = []
    ...
    monkeypatch.setattr(
        routes,
        "read_body",
        lambda _handler: {"worktree": False},
    )
    monkeypatch.setattr(routes, "new_session", lambda **_kwargs: _NewSession())
    monkeypatch.setattr(
        routes,
        "publish_session_list_changed",
        lambda event, **payload: events.append((event, payload)),
    )
    monkeypatch.setattr(routes, "_clear_session_list_cache", cleared.append)

    cap = _capture(monkeypatch)
    routes.handle_post(handler, urlparse("/api/session/new"))

    assert "ok" in cap, cap
    assert cap["ok"]["session"]["session_id"] == "plain_session"
    assert events == [
        ("session_new", {"profile": "default", "session_id": "plain_session"})
    ]
