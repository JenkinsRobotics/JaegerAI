...
def test_delete_cli_session_supports_profile_db_without_messages_table(
    tmp_path, monkeypatch
):
    state_db = tmp_path / "state.db"
    conn = sqlite3.connect(state_db)
    try:
        ...
        conn.execute(
            "INSERT INTO sessions (id, parent_session_id) VALUES ('victim', NULL)"
        )
        conn.commit()
    finally:
        conn.close()
