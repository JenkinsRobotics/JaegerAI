...
def test_cli_projection_hides_session_ids_routes_cannot_address(fake_hermes_home):
    _make_state_db(fake_hermes_home / "state.db", [
        ("roundtable-hermes:abc", "Legacy fixture", "cli"),
...
def test_archived_sidecars_do_not_consume_interactive_cli_window(
    fake_hermes_home, tmp_path, monkeypatch
):
    archived = [(f"archived-{idx:02d}", f"Archived {idx}", "cli") for idx in range(20)]
    _make_state_db(
        fake_hermes_home / "state.db",
        [*archived, ("live-older", "Home Finance work", "cli")],
    )
    conn = sqlite3.connect(fake_hermes_home / "state.db")
    try:
        for idx, (sid, _title, _source) in enumerate(archived, start=10):
            conn.execute(
                "UPDATE messages SET timestamp=? WHERE session_id=?",
                (float(idx), sid),
            )
        conn.execute(
            "UPDATE messages SET timestamp=1 WHERE session_id='live-older'"
        )
        conn.commit()
    finally:
        conn.close()
    ...
    for sid, _title, _source in archived:
        (sidecars / f"{sid}.json").write_text(
            json.dumps({"session_id": sid, "archived": True, "messages": []}),
            encoding="utf-8",
        )
