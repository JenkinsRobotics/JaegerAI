...
def test_status_json_output(capsys, monkeypatch):
    monkeypatch.setattr(lifecycle_verbs, "_container_names", lambda: ("jaeger-openclaw",))
    ...
    monkeypatch.setattr(lifecycle_verbs, "_get_containers_state", lambda: {
        "jaeger-openclaw": {"state": "running", "ip": "192.168.64.10", "image": "img"},
    })
    ...
    assert any(c["name"] == "jaeger-openclaw" and c["state"] == "running" for c in data["containers"])
...
def test_stop_dry_run(capsys, monkeypatch):
    ...
    assert "Would stop Jaeger WebUI" in captured.out
    assert f"Would bootout {lifecycle_verbs.SUPERVISOR_SERVICE}" in captured.out


def test_start_dry_run(capsys, monkeypatch):
    monkeypatch.setattr(lifecycle_verbs, "_get_launchd_jobs", lambda: {})
    monkeypatch.setattr(lifecycle_verbs, "_get_containers_state", lambda: {})
    ...
    assert "[dry-run]" in captured.out
    assert "[dry-run] Would start Jaeger WebUI" in captured.out


def test_status_checks_host_hermes_agent_native_api(monkeypatch):
    calls = []
    ...
    assert lifecycle_verbs._service_port_open(
        "com.jenkinsrobotics.hermes-native-api", 8645, {},
    ) is True
    assert calls == [("127.0.0.1", 8645)]


def test_start_does_not_rewrite_installed_profile_configuration(capsys, monkeypatch):
    monkeypatch.setattr(lifecycle_verbs, "_get_launchd_jobs", lambda: {})
    monkeypatch.setattr(lifecycle_verbs, "_get_containers_state", lambda: {})
