...
def test_native_chatbot_output_is_decoded_and_saved(monkeypatch):
    import jaeger_ai.main as app
    import jaeger_agent.loop.runtime_bridge as bridge
    ...
    seen, saved = [], []
    runtime = _runtime()
    agent = SimpleNamespace(primary_adapter=SimpleNamespace(), messages=[])
    monkeypatch.setattr(bridge, "build_jaeger_agent", lambda *a, **k: agent)
    monkeypatch.setitem(app._pipeline, "llm_lock", None)
    monkeypatch.setattr(sessions, "get_store", lambda: SimpleNamespace(
        record=lambda *a, **k: saved.append(a)))

    def drive(agent, text, *, content):
        seen.append(content)
        return {"answer": "The board shows a database.", "halt_reason": "interrupted"}

    monkeypatch.setattr(bridge, "drive_one_turn", drive)
    content = [{"type": "image_url", "image_url": {"url": "image"}}]
    result = runtime.run_chatbot_multimodal_turn(
        content, text="describe", session_key="picture", output_mode="speech")
    assert seen == [content]
    assert result["speech_text"] == "The board shows a database."
    assert result["halt_reason"] == "interrupted"
    assert saved == [("picture", "user", "describe"),
                     ("picture", "assistant", "The board shows a database.")]
