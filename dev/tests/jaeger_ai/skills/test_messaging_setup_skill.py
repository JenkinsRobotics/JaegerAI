...
Pins that ``skills/messaging/messaging-setup/SKILL.md`` parses through the
...
def test_messaging_setup_is_discovered() -> None:
    ...
    matches = [p for p in playbooks if p.name == "messaging-setup"]
    assert len(matches) == 1, "messaging-setup not discovered exactly once"
    skill = matches[0]
...
def test_messaging_setup_requires_tools_are_all_registered() -> None:
    ...
    skill = next(p for p in discover_playbooks() if p.name == "messaging-setup")
    missing = [t for t in skill.requires_tools if t not in registered]
    assert not missing, f"messaging-setup names tools that don't exist: {missing}"
