...
def test_closing_face_during_turn_does_not_kill_shared_worker(monkeypatch, tmp_path):
    from jaeger_ai import main as app
    monkeypatch.setattr(app, "run_for_voice", lambda *a, **k: {"text": "ok"})
    ctx = bridge._Ctx()
    ctx.booted.set()
    ctx.client = object()
    ...
    ctx.layout = SimpleNamespace(run_dir=tmp_path, memory_dir=tmp_path)
    turns.put({"op": "send", "text": "native chat"})
    turns.put(None)
...
def test_attached_approval_returns_only_on_the_requesting_connection(tmp_path, monkeypatch):
    ...
    ctx.client = object()
    ctx.booted.set()
    primary, turns = io.StringIO(), queue.Queue()
    provider = bridge.BridgeConfirmationProvider(primary, ctx)
    provider.TIMEOUT_S = 3
    monkeypatch.setattr(permissions, "current_policy", lambda: SimpleNamespace(confirmation=provider))
    ctx.runtime = SimpleNamespace(run_multimodal_turn=lambda *a, **k: {
        "text": provider.request("approval", "Allow the test action?", ("once", "deny"))})
    worker = threading.Thread(target=bridge._turn_worker, args=(primary, ctx, turns))
    worker.start()
    server = bridge._AttachedFaceServer(ctx, turns)
    server.start()
    client = AttachedAgentRuntime(server.path, timeout=5)
    stranger = AttachedAgentRuntime(server.path, timeout=5)
    observed = []

    def respond(frame):
        if frame.get("type") == "request":
            stranger.respond(frame["id"], "always")
            time.sleep(0.03)
            observed.append(ctx.pending[frame["id"]][1][:])
            client.respond(frame["id"], "once")

    client.set_event_sink(respond)
    try:
        result = client.run_multimodal_turn("test", text="test", system_prompt="test", session_key="approval")
        assert result["text"] == "once"
        assert observed == [[]]
        assert not ctx.pending and not ctx.pending_outputs
        assert '"type": "request"' not in primary.getvalue()
    finally:
        client.close()
        stranger.close()
        server.stop()
