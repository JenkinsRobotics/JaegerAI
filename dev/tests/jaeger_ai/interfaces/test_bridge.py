...
def test_effective_icon_prefers_instance_avatar_over_character_card(tmp_path):
    ...
    from jaeger_ai.core.instance.schemas import Identity, dump_yaml
    from jaeger_ai.interfaces import bridge
...
def test_speak_command_roundtrip(monkeypatch):
    ...
    stdin = (
        '{"op":"command","cmd":"speak","args":{"text":"Good day."},"id":"r3"}\n'
...
def test_config_query_carries_context_window_knobs(monkeypatch):
    ...
    ok, err = bridge._command(
        "save_config", {"model_ctx": 999_999}, type("B", (), {"layout": _Lay()})()
    )
...
def _write_valid_instance(root):
    ...
    dump_yaml(root / "identity.yaml", Identity(name="T", role="r", personality="p"))
