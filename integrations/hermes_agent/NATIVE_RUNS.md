...
Jaeger WebUI is the common browser interface. JaegerAI owns the profile
...
generated credentials in Git. Hermes Agent now runs from the host checkout and
its authenticated API listens only on `127.0.0.1:8645`; Jaeger WebUI is shipped
directly from `jaeger_ai/features/webui`.
