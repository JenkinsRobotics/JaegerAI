...
def _print_setup_usage() -> None:
    print(
        ...
        "\n"
        "  --name NAME  set up a DIFFERENT instance dir, e.g. `work` or\n"
        "               `personal`. The wizard will create or rebuild\n"
        "               ~/.jaeger/instances/<NAME>/. This is the\n"
        "               INSTANCE directory name (used by `jaeger -i NAME`),\n"
        "               not the agent's display name (the wizard asks\n"
        "               for that separately in Step 1).\n"
...
def _instance_inspect(argv: list[str]) -> int:
    ...
    if dist_path.exists():
        try:
            from jaeger_os.core.instance.schemas import DistributionConfig
