"""jaeger_client — the single-file Python client for a JaegerAI agent.

Copy this ONE file into your app. Stdlib only, no dependencies, no JaegerAI
package in your venv: it drives an EXISTING JaegerAI install (the ~/jaeger
...
and speaking the v1 NDJSON client protocol over stdio.
    ...
    with JaegerClient() as jaeger:                 # uses ~/jaeger (or $JAEGER_HOME)
        reply = jaeger.turn("hello", session="myapp")
        print(reply["text"])

Pick an agent:      JaegerClient(instance="lilith")
