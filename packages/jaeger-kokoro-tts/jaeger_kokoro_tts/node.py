"""Bus-facing JaegerOS node for Kokoro speech synthesis.

The bus callback only validates and queues requests. Model inference and
playback run serially on the node thread, so slow speech never blocks bus
delivery for unrelated sensors or actuators. Every accepted request receives
exactly one correlated :class:`SpokenAck`, including overload, shutdown, and
engine-failure paths.

Threading
---------
The bus delivery thread MUST NOT block on slow work; otherwise it
back-pressures every other subscriber on every other topic.  TTS
synthesis + playback takes hundreds of ms to seconds, so the
subscriber callback (``_on_speech_command``) only enqueues; the
node's own ``tick()`` drains the queue and runs synthesis serially
on the node thread.

Backpressure
------------
Internal queue is bounded (default 32 messages).  When full, the
new request gets an immediate failure ack (``ok=False``,
``reason="TTS queue full"``) so the brain's tool sees the rejection
right away rather than waiting for a timeout.
"""

from __future__ import annotations

import inspect
import queue
import threading
import time
from typing import Any, Protocol

from jaeger_os.transport import topics
from jaeger_os.nodes.base import Node
from jaeger_os.transport import Bus


class Synthesizer(Protocol):
    """The bit of :class:`KokoroTTS` the TTS node depends on.

    Production callers pass the real ``KokoroTTS`` instance.  Tests
    pass a mock that records calls and returns canned results.
    """

    def speak(
        self,
        text: str,
        *,
        voice: str | None = None,
        rate: float = 1.0,
        correlation_id: str | None = None,
    ) -> dict[str, Any]:
        """Synthesize + play ``text``; block until playback finishes.
        Returns a dict with at least ``spoken: bool``; may also carry
        ``elapsed_s``, ``reason`` (when ok=False), ``text`` (cleaned),
        ``samples``."""
        ...

    def shutdown(self) -> None:
        """Tear the synthesizer down cleanly (close audio device,
        unload model)."""
        ...


class TTSNode(Node):
    """SUB ``/act/speech/say`` → synthesize → PUB ``/act/speech/spoken``.

    The synthesizer is dependency-injected so another TTS engine can claim
    the same slot without changing this request/ack lifecycle.
    """

    def __init__(
        self,
        *,
        bus: Bus,
        synthesizer: Synthesizer,
        name: str = "tts",
        queue_maxsize: int = 32,
        max_text_chars: int = 4000,
        warm_on_start: bool = True,
        install_signal_handlers: bool = False,
    ) -> None:
        super().__init__(
            bus=bus,
            name=name,
            install_signal_handlers=install_signal_handlers,
        )
        self.synthesizer = synthesizer
        # Per-message handoff from bus delivery thread → node thread.
        self._pending: "queue.Queue[topics.SpeechCommand]" = queue.Queue(
            maxsize=queue_maxsize,
        )
        self._queue_maxsize = int(queue_maxsize)
        self._max_text_chars = int(max_text_chars)
        self._warm_on_start = bool(warm_on_start)
        self._warm_thread: threading.Thread | None = None
        self._warm_status = "disabled" if not warm_on_start else "pending"
        # Correlation id for the speech currently inside synthesizer.speak().
        # A SpeechStop carrying a different id is stale and must not cut a
        # newer utterance that happened to start after the operator's stop.
        self._active_correlation_id: str | None = None
        self._requests = 0
        self._completed = 0
        self._failed = 0
        self._rejected = 0
        self._last_duration_s = 0.0
        self._last_error: str | None = None

    # ── lifecycle ─────────────────────────────────────────────────

    def setup(self) -> None:
        self.subscribe(topics.ACT_SPEECH_SAY, self._on_speech_command)
        self.subscribe(topics.ACT_SPEECH_STOP, self._on_speech_stop)
        self._log(
            f"subscribed to {topics.ACT_SPEECH_SAY} + {topics.ACT_SPEECH_STOP}"
        )
        warm = getattr(self.synthesizer, "warm", None)
        if self._warm_on_start and callable(warm):
            self._warm_status = "warming"
            self._warm_thread = threading.Thread(
                target=self._warm_synthesizer,
                name=f"{self.name}-warm",
                daemon=True,
            )
            self._warm_thread.start()

    def tick(self) -> None:
        """Drain one queued speech request per tick.  Synthesis runs
        synchronously on this thread; the next tick won't start until
        the current speech finishes playing."""
        try:
            msg = self._pending.get(timeout=0.1)
        except queue.Empty:
            return
        self._handle(msg)

    def teardown(self) -> None:
        self.unsubscribe(topics.ACT_SPEECH_SAY, self._on_speech_command)
        self.unsubscribe(topics.ACT_SPEECH_STOP, self._on_speech_stop)
        # Do not strand callers whose accepted requests never started.
        while True:
            try:
                pending = self._pending.get_nowait()
            except queue.Empty:
                break
            self._publish_ack(
                pending,
                ok=False,
                reason="TTS node stopping",
                duration_s=0.0,
            )
        if self.synthesizer is not None:
            try:
                self.synthesizer.shutdown()
            except Exception as exc:  # noqa: BLE001
                self._log(f"synthesizer shutdown error: "
                          f"{type(exc).__name__}: {exc}")

    def stop(self) -> None:
        """Cancel active playback before waiting for the node thread.

        ``Node.stop()`` only flips the loop flag. A TTS tick can block for
        the duration of an utterance, so the synthesizer must also receive
        an immediate cancellation signal during app shutdown.
        """
        stop = getattr(self.synthesizer, "stop", None)
        if callable(stop):
            try:
                stop()
            except Exception as exc:  # noqa: BLE001
                self._log(f"synthesizer stop error: {type(exc).__name__}: {exc}")
        super().stop()

    def health(self) -> dict[str, Any]:
        health = super().health()
        health.update({
            "queue_depth": self._pending.qsize(),
            "queue_capacity": self._queue_maxsize,
            "active_correlation_id": self._active_correlation_id,
            "requests": self._requests,
            "completed": self._completed,
            "failed": self._failed,
            "rejected": self._rejected,
            "last_duration_s": self._last_duration_s,
            "last_error": self._last_error,
            "warm_status": self._warm_status,
            "voice": getattr(self.synthesizer, "voice", None),
            "lang": getattr(self.synthesizer, "lang", None),
        })
        return health

    def _warm_synthesizer(self) -> None:
        try:
            result = self.synthesizer.warm()
            self._warm_status = (
                "ready" if result.get("warmed", False) else "failed"
            )
            if self._warm_status == "failed":
                self._last_error = str(result.get("reason") or "warm failed")
        except Exception as exc:  # noqa: BLE001
            self._warm_status = "failed"
            self._last_error = f"{type(exc).__name__}: {exc}"
            self._log(f"warm failed: {self._last_error}")

    # ── subscriber + worker ──────────────────────────────────────

    def _on_speech_command(self, msg: topics.TopicMessage) -> None:
        """Called by the bus delivery thread.  Must return quickly —
        delegate slow work to the node tick()."""
        assert isinstance(msg, topics.SpeechCommand), (
            f"TTS node got unexpected topic: {msg.topic}"
        )
        self._requests += 1
        if not msg.text or not msg.text.strip():
            self._rejected += 1
            self._publish_ack(
                msg, ok=False, reason="empty text", duration_s=0.0,
            )
            return
        if len(msg.text) > self._max_text_chars:
            self._rejected += 1
            self._publish_ack(
                msg,
                ok=False,
                reason=f"text exceeds {self._max_text_chars} characters",
                duration_s=0.0,
            )
            return
        if not 0.5 <= float(msg.rate) <= 2.0:
            self._rejected += 1
            self._publish_ack(
                msg,
                ok=False,
                reason="rate must be between 0.5 and 2.0",
                duration_s=0.0,
            )
            return
        try:
            self._pending.put_nowait(msg)
        except queue.Full:
            # Immediate-fail ack so the brain's tool sees rejection.
            self._rejected += 1
            self._publish_ack(
                msg, ok=False, reason="TTS queue full", duration_s=0.0,
            )

    def _on_speech_stop(self, msg: topics.TopicMessage) -> None:
        """Barge-in primitive: the voice loop (or any other coordinator)
        publishes :class:`SpeechStop` when it wants the in-flight speech
        cut.  We forward to ``synthesizer.stop()`` if the engine
        supports it; the in-flight ``synthesizer.speak()`` returns
        promptly with ``spoken=False`` and the worker's normal
        flow publishes the SpokenAck so any bus.request() in flight
        wakes up.

        Runs on the bus delivery thread — must not block.
        ``synthesizer.stop()`` is implemented as a flag flip on
        KokoroTTS, so it's effectively instant."""
        assert isinstance(msg, topics.SpeechStop), (
            f"TTS node got unexpected topic on speech_stop subscription: "
            f"{msg.topic}"
        )
        if (
            msg.correlation_id
            and self._active_correlation_id
            and msg.correlation_id != self._active_correlation_id
        ):
            self._log(
                "speech_stop ignored for stale correlation_id "
                f"{msg.correlation_id}"
            )
            return
        if msg.correlation_id and self._active_correlation_id is None:
            self._log(
                "speech_stop ignored because no matching speech is active"
            )
            return
        stop = getattr(self.synthesizer, "stop", None)
        if not callable(stop):
            self._log(
                "speech_stop received but synthesizer has no stop(); "
                "ignoring"
            )
            return
        try:
            stop()
        except Exception as exc:  # noqa: BLE001
            self._log(f"synthesizer stop() raised: "
                      f"{type(exc).__name__}: {exc}")

        # Drain any pending queued commands that match this stop so queued
        # streaming sentences don't continue playing after an interruption.
        remaining: list[topics.SpeechCommand] = []
        while True:
            try:
                cmd = self._pending.get_nowait()
                if not msg.correlation_id or cmd.correlation_id == msg.correlation_id:
                    self._publish_ack(cmd, ok=False, duration_s=0.0, reason="interrupted by speech_stop")
                else:
                    remaining.append(cmd)
            except queue.Empty:
                break
        for cmd in remaining:
            try:
                self._pending.put_nowait(cmd)
            except queue.Full:
                break

    def _handle(self, msg: topics.SpeechCommand) -> None:
        t0 = time.perf_counter()
        self._active_correlation_id = msg.correlation_id
        # Publish measured speech amplitude while playback runs.
        amp_stop = threading.Event()
        cid_for_chunks = msg.correlation_id or ""
        amp_thread = threading.Thread(
            target=self._tts_amplitude_pulse,
            args=(amp_stop, cid_for_chunks),
            name=f"tts-amp-{cid_for_chunks[:8] or 'anon'}",
            daemon=True,
        )
        amp_thread.start()
        # SpeechCommand.voice is a per-utterance override. The node queue is
        # serialized, so changing the Kokoro pack for this blocking call and
        # restoring it afterwards cannot leak into another utterance. This is
        # what lets setup and the agent share one loaded Kokoro-82M model.
        previous_voice = getattr(self.synthesizer, "voice", None)
        voice_overridden = bool(msg.voice and hasattr(self.synthesizer, "voice"))
        if voice_overridden:
            self.synthesizer.voice = msg.voice
        try:
            result = self._speak(msg)
        except Exception as exc:  # noqa: BLE001
            self._log(f"speak() raised: {type(exc).__name__}: {exc}")
            self._publish_ack(
                msg, ok=False,
                reason=f"{type(exc).__name__}: {exc}",
                duration_s=time.perf_counter() - t0,
            )
            return
        finally:
            if voice_overridden:
                self.synthesizer.voice = previous_voice
            amp_stop.set()
            amp_thread.join(timeout=0.5)
            self._active_correlation_id = None
            # Always close the mouth, including inference exceptions and
            # cancellation paths.
            try:
                self.publish(topics.TtsChunk(
                    amplitude=0.0,
                    is_final=True,
                    node_id=self.name,
                    correlation_id=cid_for_chunks,
                ))
            except Exception:  # noqa: BLE001
                pass
        ok = bool(result.get("spoken", False))
        # Prefer the synthesizer's elapsed if it gives one (more
        # accurate — includes synth time, not just playback) but
        # fall back to wall-clock so the brain always gets a number.
        duration_s = float(
            result.get("elapsed_s") or (time.perf_counter() - t0)
        )
        reason = result.get("reason") if not ok else None
        self._publish_ack(msg, ok=ok, duration_s=duration_s, reason=reason)

    def _speak(self, msg: topics.SpeechCommand) -> dict[str, Any]:
        """Call the standard synthesizer API while tolerating old adapters.

        ``Synthesizer`` now supports request-local voice, speed, and
        correlation. Signature inspection keeps simple third-party engines
        that still implement only ``speak(text)`` usable without catching a
        real ``TypeError`` raised from inside their inference code.
        """
        speak = self.synthesizer.speak
        parameters = inspect.signature(speak).parameters
        accepts_kwargs = any(
            p.kind is inspect.Parameter.VAR_KEYWORD
            for p in parameters.values()
        )
        values = {
            "voice": msg.voice or None,
            "rate": float(msg.rate),
            "correlation_id": msg.correlation_id,
        }
        kwargs = {
            key: value for key, value in values.items()
            if accepts_kwargs or key in parameters
        }
        return speak(msg.text, **kwargs)

    def _tts_amplitude_pulse(
        self,
        stop_event: threading.Event,
        correlation_id: str,
    ) -> None:
        """Publish real output RMS at 30 Hz; missing telemetry means silence."""
        interval_s = 1.0 / 30.0
        while not stop_event.is_set():
            amplitude = max(0.0, min(1.0, float(getattr(self.synthesizer, "amplitude", 0.0))))
            try:
                self.publish(topics.TtsChunk(
                    amplitude=amplitude,
                    is_final=False,
                    node_id=self.name,
                    correlation_id=correlation_id,
                ))
            except Exception:  # noqa: BLE001
                return
            stop_event.wait(timeout=interval_s)

    def _publish_ack(
        self,
        request: topics.SpeechCommand,
        *,
        ok: bool,
        duration_s: float,
        reason: str | None,
    ) -> None:
        self._last_duration_s = float(duration_s)
        if ok:
            self._completed += 1
            self._last_error = None
        else:
            self._failed += 1
            self._last_error = reason
        self.publish(topics.SpokenAck(
            ok=ok,
            duration_s=duration_s,
            reason=reason,
            node_id=self.name,
            correlation_id=request.correlation_id,
        ))
