"""Smoke test for the whisper_stt engine (``jaeger_whisper_stt/engine/``).
...
the plugin into ``jaeger_os/jaeger_whisper_stt/engine/``.
"""
...
def test_default_alias_is_two_pass() -> None:
    from jaeger_whisper_stt.engine import WhisperSTT, WhisperSTTTwoPass
...
def test_both_modes_importable() -> None:
    ...
    from jaeger_whisper_stt.engine import (
...
def test_shared_helpers() -> None:
    ...
    from jaeger_whisper_stt.engine._base import (
...
def test_unknown_method_is_a_configuration_error() -> None:
    from jaeger_whisper_stt.engine.registry import get

    try:
        get("two_pas")
    except ValueError as exc:
        assert "unknown STT method" in str(exc)
        assert "two_pass" in str(exc)
    else:
        raise AssertionError("a misspelled production mode silently fell back")


def test_realtime_output_queue_is_bounded_newest_wins() -> None:
    import queue

    from jaeger_whisper_stt.engine._base import offer_latest

    q = queue.Queue(maxsize=2)
    assert offer_latest(q, "old") is False
    assert offer_latest(q, "middle") is False
    assert offer_latest(q, "new") is True
    assert [q.get_nowait(), q.get_nowait()] == ["middle", "new"]
