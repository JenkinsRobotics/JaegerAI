...
def test_release_package_data_is_an_explicit_allowlist() -> None:
    """Do not ship tests or machine-specific bytecode in the wheel.

    A recursive package-data glob follows whatever happens to be present in
    the source tree at build time, including ``tests`` and ``__pycache__``.
    Production artifacts must instead name their required non-Python assets.
    """
    project = tomllib.loads((PACKAGE.parent / "pyproject.toml").read_text())
    setuptools = project["tool"]["setuptools"]
    assert setuptools["include-package-data"] is False
    assert setuptools["package-data"]["jaeger_whisper_stt"] == ["module.yaml"]
    assert "jaeger_whisper_stt.tests*" in setuptools["packages"]["find"]["exclude"]
...
def test_manifest_declares_stable_identity() -> None:
    assert MANIFEST["id"] == "org.jenkinsrobotics.stt.whisper"
    assert MANIFEST["name"] == "JaegerWhisperSTT"
    assert MANIFEST["type"] == "module"
    assert MANIFEST["implementation"] == "whisper"
