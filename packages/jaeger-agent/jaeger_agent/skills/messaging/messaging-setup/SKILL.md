...
description: Bring a messaging channel (Discord, Telegram, iMessage) live so the agent
  can send/receive on it. Load this whenever the user wants to 'connect Discord',
  'set up Telegram', 'text me on iMessage', asks why a channel isn't working, or asks
  what's needed to message them somewhere new.
metadata:
  jros:
    aliases:
    - messaging_setup
