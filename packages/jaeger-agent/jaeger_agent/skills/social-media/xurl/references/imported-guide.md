...
```bash
# Download, inspect, and then run the official installer as separate steps.
curl -fsSLo /tmp/xurl-install.sh https://raw.githubusercontent.com/xdevplatform/xurl/main/install.sh
...
- **Token storage:** xurl manages its own credential file. Never read, display, or
  transmit that file through the agent.
