...
def _plugins_root() -> pathlib.Path:
    """Where ``plugin.yaml`` directories live — the HOST's, not ours.

    This was ``parents[2] / "plugins"`` when the file sat in
    ``jaeger_ai/agent/tools/``, which resolved to ``jaeger_ai/plugins``.
    After the 0.11 move the same expression points at the jaeger-agent
    repo root, where no such directory exists — so plugin discovery
    silently returned nothing rather than failing. Resolving it from the
    host package keeps it pointing at the real one.
