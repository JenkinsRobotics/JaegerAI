"""Core shared infrastructure for every Jaeger tool module.
...
if TYPE_CHECKING:  # annotations only — no runtime dependency on a host
    from .instance import Layout as InstanceLayout
...
class DefaultWorkspace:
    ...
    Default state lives below the external operator state directory. A host
    can pass an explicit root for an isolated instance or project workspace.
    """

    def __init__(self, root: Path | str | None = None) -> None:
        import os
        state_root = (os.environ.get("JAEGER_STATE_DIR") or os.environ.get("JAEGER_HOME")
                      or str(Path.home() / ".jaeger"))
        self.root = Path(root or Path(state_root) / "agent").expanduser().resolve()
...
def _require_layout() -> InstanceLayout:
    ...
    :class:`~jaeger_agent.core.runtime.DefaultAgentRuntime` calls
    :func:`ensure_bound` in its constructor. An application binds its
    own instance instead and never reaches either path.
    """
    if _layout is None:
        raise RuntimeError(
            "no workspace bound — call jaeger_agent.core.workspace.ensure_bound() "
            "for a default under the operator state directory, or bind(layout) to use "
            "your own"
        )
    return _layout
