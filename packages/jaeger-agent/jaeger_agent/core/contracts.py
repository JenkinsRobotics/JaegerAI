"""Application-neutral core contracts between JaegerAgent and a runtime."""
...
class AgentRuntime(Protocol):
    ...
    ``steer(text)``, ``context_detail(session)``, ``health()``,
    ``run_multimodal_turn(...)``, ``configure_vision(handler)``, and
    ``clear_session(session)``; JaegerAgent discovers those optional lifecycle
    hooks without coupling to one engine.
    """
