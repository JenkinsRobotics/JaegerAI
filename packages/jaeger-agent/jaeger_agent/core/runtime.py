"""The built-in core runtime — a brain from configuration alone.
...
satisfy the same :class:`~jaeger_agent.core.contracts.AgentRuntime` protocol,
so :class:`~jaeger_agent.core.node.MindNode` never knows which it got.
...
class DefaultAgentRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        #
        # Defaults to the external operator state root (~/.jaeger/agent).
        # A host that binds its own instance keeps it; ensure_bound is idempotent.
    ...
    def adapter(self) -> Any:
        ...
        self._apply_vision_handler(self._adapter)
        return self._adapter

    def _apply_vision_handler(self, adapter: Any) -> None:
        handler = self._vision_handler
        if handler is None:
            return
        configure = getattr(adapter, "configure_vision", None)
        if callable(configure):
            configure(handler)
            return
        llama_kwargs = getattr(adapter, "llama_kwargs", None)
        if isinstance(llama_kwargs, dict) and getattr(adapter, "_llama", None) is None:
            llama_kwargs["chat_handler"] = handler

    def configure_vision(self, chat_handler: Any) -> None:
        """Attach a local llama.cpp projector before the first model call."""
        self._vision_handler = chat_handler
        if self._adapter is not None:
            self._apply_vision_handler(self._adapter)

    def agent_for(self, session_key: str, *, system_prompt: str = "") -> JaegerAgent:
        ...
        if agent is None:
            agent = JaegerAgent(
                ...
                # ``None`` preserves the existing fully agentic registry path.
                # An explicit empty list locks the chatbot path to zero tools
                # without changing global registration or the agentic default.
                tools=None if self.config.tools_enabled else [],
    ...
    def warmup(
        ...
        *,
        ...
        """Warm an in-process adapter without adding a fake conversation turn.

        Cloud/served adapters deliberately have no warmup hook: issuing a
        synthetic billable request at application startup would be surprising.
        Local adapters can compile kernels, allocate their graph, and prefill the
        stable system/tool prefix here while the agent transcript stays empty.
        """
        if self._closed:
            raise RuntimeError("runtime is closed")
        agent = self.agent_for(session_key, system_prompt=system_prompt)
        warm = getattr(agent.primary_adapter, "warmup", None)
        if not callable(warm):
            return False
        warm(system_prompt=agent.system_prompt, tools=agent.tools)
        return True
    ...
    def _run_bound_turn(self, session_key: str, text: str, *, content: Any = None) -> str:
        ...
        return TurnExecutive(
            ...
            world_event=WorldEvent.for_session(text, session_key),
        ).run_turn(text, **options)
    ...
    def run_multimodal_turn(
        ...
        *,
        ...
        """Run image+text or structured-text content through the same agent.

        The visible ``text`` drives routing/verification while ``content`` is
        the exact provider payload.  Both land in the runtime-owned transcript;
        no framework history is involved.
        """
        if self._closed:
            return TurnResult(error="runtime is closed")
        try:
            agent = self.agent_for(session_key, system_prompt=system_prompt)
            return TurnResult(text=self._run_bound_turn(session_key, text, content=content))
        except Exception as exc:  # noqa: BLE001 — match run_turn's boundary
            return TurnResult(error=f"{type(exc).__name__}: {exc}")

    def clear_session(self, session_key: str) -> None:
        """Discard one conversation without closing the shared runtime."""
        agent = self._sessions.pop(session_key, None)
    ...
    def interrupt(self, *, session_key: str) -> None:
        """Cancel only the selected session; do not construct an idle agent."""
        agent = self._sessions.get(session_key)
        if agent is not None:
            agent.interrupt()
