"""Core agent-side safety — the pillars that REQUIRE an agent to interpret.

Two of the four safety pillars need an LLM, not deterministic code, so they
live here in the agent's ``core/`` rather than JaegerOS's deterministic
``jaeger_os/core/safety/`` package:
...
The DETERMINISTIC pillars stay in ``jaeger_os/core/safety/`` because they fire
...
_THREE_LAWS_PATH = Path(__file__).parent.parent / "prompts" / "three_laws.md"
