...
async function checkUpdatesNow(channelOverride){
  ...
  try {
    ...
    if(data.disabled){
      ...
      const webuiError=formatUpdateError('Hermes WebUI',data.webui);
      const agentError=formatUpdateError('Hermes Agent',data.agent);
      ...
      const webuiPart=formatUpdatePart('Hermes WebUI',data.webui);
      const agentPart=formatUpdatePart('Hermes Agent',data.agent);
      ...
      const noGitParts=[];
      if(data.webui&&data.webui.no_git&&!data.webui.manual_update) noGitParts.push('Hermes WebUI');
      if(data.agent&&data.agent.no_git&&!data.agent.ignored) noGitParts.push('Hermes Agent');
      if(parts.length){
