...
def _load_cli_sessions_uncached(
    ...
    *,
    ...
    _interactive_limit = visible_session_limit if visible_session_limit is not None else (
    ...
    # Archived CLI rows have materialized WebUI sidecars and are supplied by
    # the sidecar catalogue. Scan past them here so 20 archived fixtures cannot
    # consume the entire interactive state.db window and hide older live work.
    _state_scan_limit = (
        _interactive_limit * 5
        if source_filter is None and _interactive_limit is not None
        else _interactive_limit
    )
    _interactive_rows_added = 0
    for row in read_importable_agent_session_rows(
        db_path,
        limit=_state_scan_limit,
        log=logger,
        ...
        sid = row['id']
        # Never project a row the detail/mutation routes cannot address. Legacy
        # or fixture databases can contain IDs outside the current path-safety
        # contract; showing them creates an undeletable, unarchivable sidebar
        # ghost because every session endpoint correctly rejects the ID.
        if not is_safe_session_id(sid):
            continue
        ...
        if _archived and source_filter is None:
            continue
        ...
        _interactive_rows_added += 1
        if (
            source_filter is None
            and _interactive_limit is not None
            and _interactive_rows_added >= _interactive_limit
        ):
            break
