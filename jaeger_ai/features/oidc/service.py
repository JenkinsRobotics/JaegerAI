"""OIDC Authorization Code + PKCE support for Jaeger's remote surfaces.

Configuration is environment-owned so no provider secret is written into the
repository or a browser response. Every login is allow-list gated.
"""
...
def _resolve_oidc_config() -> dict[str, Any]:
    def pick(env_name: str) -> str:
        return os.getenv(env_name, "").strip()

    scopes = _normalize_scopes(pick("JAEGER_OIDC_SCOPES"))
    allow_values = _normalize_allow_values(
        pick("JAEGER_OIDC_ALLOW_VALUES")
    )
    return {
        "issuer": pick("JAEGER_OIDC_ISSUER").rstrip("/"),
        "client_id": pick("JAEGER_OIDC_CLIENT_ID"),
        "client_secret": pick("JAEGER_OIDC_CLIENT_SECRET"),
        "redirect_uri": pick("JAEGER_OIDC_REDIRECT_URI"),
        "scopes": scopes,
        "allow_claim": pick("JAEGER_OIDC_ALLOW_CLAIM"),
...
def _require_oidc_config() -> dict[str, Any]:
    ...
    if not cfg.get("allow_claim") or not cfg.get("allow_values"):
        raise OIDCConfigError(
            "Native OIDC login requires JAEGER_OIDC_ALLOW_CLAIM and "
