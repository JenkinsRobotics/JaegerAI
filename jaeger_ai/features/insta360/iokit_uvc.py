    ...
    def close(self):
        ...
        for handle in (self.iface_pp, self.plugin_pp):
            ...
            try:
                ...
                logger.debug("failed to release Insta360 IOKit handle", exc_info=exc)
        self.iface_pp = 0
        self.plugin_pp = 0
...
KNOWN_PIDS = (0x4c04, 0x4003, 0x4c01, 0x4001)

def open_vc_interface(vid: int = 0x2e1a, pid: int | None = None,
    ...
    try:
        while True:
            ...
            pid_match = (p == pid) if pid is not None else (p in KNOWN_PIDS)
            if v == vid and pid_match and n == iface_num:
                service = svc
                break
...
class Link2GimbalIOKit:
    """High-level hardware controller for Insta360 Link 2 gimbal via IOKit."""

    def __init__(self, pid: int | None = None):
        self.pid = pid
        self._handle: VCInterface | None = None

    def connect(self) -> bool:
        if self._handle and not self._handle.is_closed:
            return True
        self._handle = open_vc_interface(pid=self.pid)
    ...
    def read_pan_tilt(self) -> tuple[int, int]:
        """Read absolute gimbal position: returns (pan, tilt)."""
        if not self.connect() or not self._handle:
            raise ConnectionError("Insta360 Link 2 device not found on USB")
        raw = ctrl_get(self._handle, 9, 0x1A, 8)
        tilt, pan = struct.unpack("<ii", raw)
        return pan, tilt

    def write_pan_tilt(self, pan: int, tilt: int) -> bool:
        """Write target pan/tilt position."""
        if not self.connect() or not self._handle:
            raise ConnectionError("Insta360 Link 2 device not found on USB")
        data = struct.pack("<ii", tilt, pan)
        ctrl_set(self._handle, 9, 0x1A, data)
        return True

    def read_func_enable(self) -> bytes:
        """Read functional enable bitmask."""
        if not self.connect() or not self._handle:
            raise ConnectionError("Insta360 Link 2 device not found on USB")
        return ctrl_get(self._handle, 9, 0x1B, 2)
