"""Passkey/WebAuthn helpers for the Hermes WebUI adapter.
...
except ImportError:  # pragma: no cover - exercised by source tests instead
...
_RP_NAME = "JaegerAI Hermes WebUI Adapter"


def bind_state_dir(path: Path) -> None:
    """Bind passkey state to one Jaeger instance before serving requests."""
    global _STATE_DIR
    _STATE_DIR = Path(path).resolve()


def _state_file(name: str) -> Path:
    if _STATE_DIR is None:
        raise PasskeyError("Passkey state is not bound to a Jaeger instance")
...
def _load_credentials() -> list[dict[str, Any]]:
    data = _json_load(_state_file("passkeys.json"), [])
...
def _save_credentials(creds: list[dict[str, Any]]) -> None:
    _atomic_write_json(_state_file("passkeys.json"), creds)
...
def _load_challenges() -> dict[str, dict[str, Any]]:
    ...
    challenges_file = _state_file(".passkey_challenges.json")
    raw = _json_load(challenges_file, {})
    ...
    if clean != raw:
        _atomic_write_json(challenges_file, clean)
    return clean
...
def _store_challenge(challenge: str, kind: str, rp_id: str, origin: str) -> None:
    with _CHALLENGES_LOCK:
        ...
        _atomic_write_json(_state_file(".passkey_challenges.json"), data)
...
def rp_context(handler) -> tuple[str, str]:
    ...
    if proto not in {"http", "https"}:
        proto = "https" if bool(getattr(handler, "is_secure_context", False)) else "http"
...
def registration_options(handler) -> dict[str, Any]:
    ...
    return {
        ...
        "user": {
            "id": _b64u(hashlib.sha256(rp_id.encode()).digest()[:16]),
            "name": "JaegerAI",
...
def clear_credentials() -> None:
    ...
    if _state_file("passkeys.json").exists():
        _save_credentials([])
