# Research Paper Pipeline — Detailed Step-by-Step Playbook

Long-form companion to `../SKILL.md`. Load with `read_file("references/pipeline-detailed.md")`
when you need the full step detail for a phase. Tool calls here use REAL JROS tools
(terminal, execute_code, read_file, write_file, patch, web_search, web_extract, todo,
memory, start_background/check_background, schedule_prompt). For the JROS tool map see
`references/jros-tooling.md`.
