...
from jaeger_ai.agent.loop.interrupt import AgentInterrupted, StaleCallTimeout
from jaeger_ai.agent.loop import verify_gate
...
from jaeger_ai.agent.util.context_guard import ContextGuard, ContextOverflow
...
class JaegerAgent:
    ...
    def run_turn(self, user_message: str) -> str:
        ...
        **Exception contract.** ``self.messages`` is left well-formed
        ...
        try:
            from jaeger_ai.agent.tools.files import reset_read_tracker
            reset_read_tracker()
...
def _classify_adapter_error(exc: BaseException) -> str:
    ...
    try:
        from jaeger_ai.core.runtime.cloud_errors import classify_exception
...
def _retry_delay(kind: str, attempt: int) -> float:
    ...
    immediately (~1-2s)."""
    from jaeger_ai.agent.util.retry_utils import jittered_backoff
    if kind == "rate_limit":
