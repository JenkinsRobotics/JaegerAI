...
def dispatch(argv: Sequence[str]) -> int:
    ...
    if argv[0] == "agent":
        from jaeger_ai.cli.verbs.instance_verbs import _cmd_agent_argv
    ...
    if argv[0] == "backup":
        from jaeger_ai.cli.verbs.backup_restore import _cmd_backup_argv
        return _cmd_backup_argv(list(argv[1:]))
    if argv[0] == "restore":
        from jaeger_ai.cli.verbs.backup_restore import _cmd_restore_argv
        return _cmd_restore_argv(list(argv[1:]))
    if argv[0] == "update":
        from jaeger_ai.cli.verbs.update_verb import _cmd_update_argv
        return _cmd_update_argv(list(argv[1:]))
    if argv[0] == "reinstall":
        from jaeger_ai.cli.verbs.update_verb import _cmd_reinstall_argv
        return _cmd_reinstall_argv(list(argv[1:]))
    if argv[0] == "uninstall":
        from jaeger_ai.cli.verbs.uninstall_verb import _cmd_uninstall_argv
        return _cmd_uninstall_argv(list(argv[1:]))
    if argv[0] == "autostart":
        from jaeger_ai.cli.verbs.autostart_verb import _cmd_autostart_argv
        return _cmd_autostart_argv(list(argv[1:]))
    if argv[0] == "launcher":
        from jaeger_ai.cli.verbs.launcher_verb import _cmd_launcher_argv
        return _cmd_launcher_argv(list(argv[1:]))
    if argv[0] == "skill":
        from jaeger_ai.cli.verbs.skill_verbs import _cmd_skill_argv
    ...
    if argv[0] == "memory":
        from jaeger_ai.cli.verbs.memory_verbs import _cmd_memory_argv
    ...
    if argv[0] == "voice":
        from jaeger_ai.features.voice.__main__ import main as voice_main
        return voice_main(list(argv[1:]))
...
def _cmd_bench(argv: list[str]) -> int:
    ...
    if verb == "compare":
        from jaeger_ai.cli.verbs.bench_compare_verb import _cmd_bench_compare_argv
        return _cmd_bench_compare_argv(rest)

    if verb == "history":
        from jaeger_ai.cli.verbs.bench_history_verb import _cmd_bench_history_argv
...
def _print_usage() -> None:
    print(
        ...
        "  update   Upgrade Jaeger AI and migrate stale instances.\n"
        ...
        "  voice       Talk to the resident Entity (microphone, WAV, or typed).\n"
