...
def _register_builtins(client: Any) -> None:
    ...
    # Agent tools that queue background work reach the Gateway task owner through
    # the agent package's task port; this host is what fills it in.
    from jaeger_agent import task_port
    from jaeger_ai.core.tasks import client as _task_client

    task_port.install_remote(_task_client)
    ...
    def adjust_trait(layer: str, field: str, value: float) -> dict:
        ...
        from jaeger_ai.features.personality.character import active_character, layer_items
    ...
    def diagnostics() -> dict:
        ...
        from jaeger_ai.core.runtime import readiness
    ...
    from jaeger_ai.plugins import homeassistant as _homeassistant  # noqa: F401
...
# ---------------------------------------------------------------------------
# Sub-agent execution lives in its own module — see core/runtime/subagents.py.
# _delegate_role and _llm_lock_held are threading.local() objects shared with it
# by identity; rebinding either here would silently split the two halves.
from jaeger_ai.core.runtime.subagents import (  # noqa: E402
    _DELEGATE_MAX_DEPTH,
    _delegate_background,
    _delegate_depth,
    _delegate_external,
    _delegate_internal,
    _delegate_parallel,
...
def _preflight_log() -> None:
    ...
    try:
        from jaeger_ai.core.runtime.preflight import boot_warning, check_environment
...
def _persona_identity_block(agent_name: str, character: Any) -> str:
    ...
    :func:`jaeger_ai.features.personality.character.persona_display_name` for the
...
def _ensure_session_agent(client: Any, session_key: str, *, scope_tools: bool | None = None) -> Any:
    ...
    if key not in _jaeger_agents_by_session:
        ...
        def _tool_progress(name: str, phase: str, data: Any) -> None:
            from jaeger_ai.plugins.registry import fire_hook
            if phase == "start":
        ...
        from jaeger_ai.features.dispatcher.store import session_policy, context_note
    ...
    try:
        ...
        from jaeger_ai.features.dispatcher.store import session_policy
...
def _run_subordinate_react(
    ...
    *,
    ...
    :func:`_run_actionable_turn`). ``content``/``system_prompt_addon`` carry
    multimodal media and persona instructions through to the loop."""
    ...
    actionable = _run_actionable_turn(
        ...
        allow_persona=allow_persona, request=request, **options,
    )
...
def _run_direct_model_runner(
    ...
    *,
    ...
    with tool_allowlist([]):
        ...
        result = drive_one_turn(agent, prompt, **({"content": content} if content is not None else {}))
    ...
    if "unknown tool" in text.lower() or ("<" in text and "tool" in text.lower()):
        ...
        with tool_allowlist([]):
            agent = build_jaeger_agent(client, tools=[], skip_final_tools=set())
            result = drive_one_turn(agent, retry, **({"content": content} if content is not None else {}))
    ...
    *,
    ...
    if is_subordinate:
        return _run_subordinate_react(
            ...
            content=content, system_prompt_addon=system_prompt_addon,
        )
    ...
    try:
        ...
        context = {
            ...
            "react_runner": lambda t, session_key=session_key, _c=content, _a=system_prompt_addon: _run_subordinate_react(
                client, t, session_key=session_key, allow_persona=False,
                request=user_text, content=_c, system_prompt_addon=_a,
            ),
            "model_runner": lambda t, _c=content: _run_direct_model_runner(
...
def _refresh_character_prompt(jaeger_agent: Any) -> None:
    ...
    an actual change."""
    try:
        from jaeger_ai.features.personality.character import active_character_signature
    ...
    if not engine_owned and output_mode is not None:
        ...
        with multimodal_output_scope():
            out = _run_turn(
                ...
                request_id=request_id, is_subordinate=is_subordinate,
            )
        ...
        out = _run_turn(
            client, user_text, session_key=session,
            request_id=request_id, is_subordinate=is_subordinate,
        )
    else:
        out = _run_turn(
    ...
    def _aux_lane(self) -> Any:
        ...
        with self._aux_lock:
            if self._aux_llm is None:
                try:
                    from jaeger_ai.core.models.aux_lane import spawn_aux_context
...
def make_client(config: Any, layout: Any = None, *, warmup: bool = True) -> Any:
    ...
    from jaeger_ai.core.models.engine_registry import resolve_engine
...
def self_test(layout: InstanceLayout) -> int:
    ...
    try:
        from jaeger_ai.plugins.registry import discover_plugins
    ...
    try:
        from jaeger_ai.core.instance.migrations import discover_migrations
...
def _cli_create_instance(name: str, *, force: bool = False) -> int:
    ...
    from jaeger_ai.core.models.model_resolver import DEFAULT_MODEL
...
def run_daemon(*, instance_name: str | None = None,
    ...
    """Run the canonical Gateway owner; background work is never a second daemon."""
    if instance_name:
        os.environ['JAEGER_INSTANCE_NAME'] = instance_name
    from jaeger_ai.core.gateway.server import main as gateway_main
    return gateway_main([])
...
def _main_dispatch() -> int:
    ...
    from jaeger_ai.cli.verbs.dispatch import dispatch as _verb_dispatch, is_daemon_subcommand as _is_verb
    ...
    if getattr(args, "doctor", False):
        from jaeger_ai.core.diagnostics import run_doctor
