...
Generated by `dev/scripts/generate_agent_contract.py` from `packages/jaeger-agent/jaeger_agent/prompts/`. Do not hand-edit.

These are the shared prompt sources. The final prompt also includes instance identity, tool schemas, runtime context, and one of the two toolset notes below.
