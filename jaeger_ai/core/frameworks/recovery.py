...
from jaeger_ai.contract.frameworks import SOLO_RUNTIMES, canonical_runtime
...
def reconcile_owned_run(runtime: str, run_id: str, *, state_root: Path | None = None) -> dict:
    ...
    if runtime == "roundtable":
        ...
        registration = backend(runtime)
        result = Runs(
            roots[0], registration.turn, reconciler=registration.reconcile
        ).reconcile(run_id)
