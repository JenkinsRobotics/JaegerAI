...
def _check_plugin_manifests() -> list[Check]:
    ...
    try:
        import pathlib
        from jaeger_ai.plugins.manifest import audit_plugin_dir
        plugins_root = (
...
def _check_tool_registry() -> list[Check]:
    ...
    ``jaeger_agent.tools.__init__``. So we need the LIVE agent
    ...
    out: list[Check] = []
    try:
        from jaeger_agent.skill_registry.toolset_scoping import CORE, LEAN_CORE, tool_visible
    except Exception as exc:  # noqa: BLE001
        out.append(Check(
