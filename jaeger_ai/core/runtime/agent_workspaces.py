...
HERMES_IMAGE = "hermes-webui:jaeger-mac-workspaces-20260906"
...
def container_name(role: str) -> str:
    if STATE_PATH.exists():
        ...
        if name:
            if name not in (LEGACY_CONTAINERS[role], MANAGED_CONTAINERS[role]):
...
def create_arguments(config: dict, role: str, *, home: Path | None = None, include_personal: bool = False) -> list[str]:
    ...
    """
    if config.get("id") != LEGACY_CONTAINERS[role]:
        raise ValueError("Migration source must be the expected legacy container")
    ...
    if role == "hermes":
        image = HERMES_IMAGE
    args = ["create", "--name", MANAGED_CONTAINERS[role], "--user", user,
