...
def _webui_unification_checks() -> list[Any]:
    """Verify the single WebUI URL and host Hermes Agent API topology."""
    from jaeger_ai.core.runtime.preflight import Check
    ...
    import socket
    try:
        with socket.create_connection(("127.0.0.1", 8645), timeout=0.5):
            host_api = True
    except OSError:
        host_api = False
    checks.append(Check(
        name="hermes_agent_host_api",
        category="webui",
        ok=host_api,
        detail="Hermes Agent native API on 127.0.0.1:8645" if host_api else "Hermes Agent native API is offline",
    ))
    return checks
