...
def _jaeger_executable() -> list[str]:
    ...
    if on_path:
        return [on_path]
    return [sys.executable, "-m", "jaeger_ai"]
...
def _existing_tui_pid_for(instance: str | None) -> int | None:
    ...
    try:
        ...
        from jaeger_ai.interfaces.tui.singleton import existing_tui_pid
...
def _make_actions(instance: str | None) -> TrayActions:
    ...
    def open_web() -> None:
        ...
        _spawn(["open", WebUIService(instance).browser_url()])
...
class MacosTray:
    ...
    def run(self) -> None:
        ...
        try:
            from jaeger_ai import __version__ as _ver
        except Exception:  # noqa: BLE001
            _ver = "unknown"
...
def _kill_stray_trays() -> int:
    ...
    for line in (out.stdout or "").splitlines():
        ...
        if pid == my_pid:
            continue
        if "jaeger_ai.interfaces.pyside6.tray" not in cmd and \
...
def main(argv: list[str] | None = None) -> int:
    ...
    from jaeger_ai.interfaces.pyside6.tray.singleton import claim_tray_slot
