...
class MultimodalWindow(QMainWindow):
    ...
    def _connect_workspace_bus(self) -> None:
        ...
        try:
            ...
            self._workspace_bridge = make_bus_bridge(
                bus,
                ["/sense/tool", "/sense/activity", "/sense/agent_state", "/sense/request"],
            )
    ...
    def _on_agent_event(self, message: Any) -> None:
        ...
        if topic == "/sense/request":
            ...
            answer = self._answer_agent_request(
                getattr(message, "kind", "approval"), getattr(message, "prompt", ""),
                list(getattr(message, "options", ()) or []))
            self.ctx.bus.publish(AgentResponse(
                id=getattr(message, "id", ""), answer=answer, session=session))
        elif topic == "/sense/tool":
    ...
    def _on_remote_agent_event(self, frame: object) -> None:
        ...
        if kind == "request":
            answer = self._answer_agent_request(
                str(frame.get("kind") or "approval"), str(frame.get("prompt") or ""),
                list(frame.get("options") or []))
            self._runtime().respond(str(frame.get("id") or ""), answer)
        elif kind == "tool":
    ...
    def _answer_agent_request(self, kind: str, prompt: str, options: list[str]) -> str:
        from PySide6.QtWidgets import QInputDialog, QMessageBox
        if kind == "secret" or (kind == "clarify" and not options):
            echo = QLineEdit.EchoMode.Password if kind == "secret" else QLineEdit.EchoMode.Normal
            text, ok = QInputDialog.getText(self, "Agent request", prompt, echo)
            return text if ok else ""
        box = QMessageBox(self)
        box.setWindowTitle("Agent request")
        box.setText(prompt)
        choices = {}
        for option in options or ["allow", "deny"]:
            role = (QMessageBox.ButtonRole.RejectRole if option == "deny"
                    else QMessageBox.ButtonRole.AcceptRole)
            choices[box.addButton(option.capitalize(), role)] = option
        box.exec()
        return choices.get(box.clickedButton(), "deny")
