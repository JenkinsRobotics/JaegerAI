...
def run_surface(ctx: Any, spec: Any) -> Any:
    ...
    only dispatches ``event_loop = "qt" | "none"``; Jaeger AI's TUI is
    its own loop launched directly by the CLI. J5B routes the
    manifest to ``event_loop = "none"`` so the chassis boots
    instance/bus/supervisor + atexit teardown, then returns control
    so Jaeger AI keeps owning the TUI loop.
    """
    raise NotImplementedError(
        "Jaeger AI TUI surface is launched directly by the CLI; the "
        "format-0.1 chassis sets event_loop = 'none' and returns "
        "after boot, leaving the TUI loop to Jaeger AI. This stub exists "
