    ...
    func register(_ handler: @escaping () -> Void) {
        ...
        let installStatus = InstallEventHandler(
            ...
            { _, eventRef, userData -> OSStatus in
                ...
                // ``"JaegerAI"`` four-char code we registered with.
                ...
                guard paramStatus == noErr,
                      hotKeyID.signature == OSType(fourCharCode("JaegerAI")),
                      hotKeyID.id == 1
                else {
        ...
        guard installStatus == noErr else {
            NSLog("[JaegerAI][hotkey] InstallEventHandler FAILED: \(installStatus)")
            eventHandlerRef = nil
            self.handler = nil
            return
        }
        ...
        let id = EventHotKeyID(
            signature: OSType(fourCharCode("JaegerAI")),
            id: UInt32(1)
        )
        ...
        if status == noErr {
            hotKeyRef = ref
            NSLog("[JaegerAI][hotkey] ⌥Space registered")
            return
        }
        ...
        // dead state.
        NSLog("[JaegerAI][hotkey] ⌥Space registration FAILED: \(status)"
...
/// "JaegerAI" tag into a big-endian UInt32 for that purpose.
