...
//  JaegerAI
//
//  Entry point for the JaegerAI desktop app.
//
...
struct JaegerAIApp: App {
    ...
    var body: some Scene {
        ...
        // Label is the JaegerAI J mark — the same artwork the 0.2.x tray
    ...
    private static func loadIcon(name: String) -> NSImage? {
        guard let url = Bundle.module.url(forResource: name,
            ...
            NSLog("[JaegerAI] icon load FAILED — \(name).png not found "
                  + "in Bundle.module (\(Bundle.module.bundlePath))")
            return nil
        }
        guard let img = NSImage(contentsOf: url) else {
            NSLog("[JaegerAI] icon load FAILED — NSImage(contentsOf:) "
                  + "returned nil for \(url.path)")
            return nil
        }
        img.size = NSSize(width: 18, height: 18)
        NSLog("[JaegerAI] icon loaded: \(name).png (size=18×18pt)")
        return img
    }
}
