...
install root (`$JAEGER_HOME`, default `~/JaegerAI`), and run the in-repo
`./install.sh` (which makes the `.venv` and does `uv pip install -e .`). On the
first 0.12+ install it detects legacy `~/jaeger`, copies `.jaeger_os/` into the
new tree while the old app is stopped, and retains the old tree for rollback.
It never overwrites `.venv/` or `.jaeger_os/` on an ordinary re-run.
